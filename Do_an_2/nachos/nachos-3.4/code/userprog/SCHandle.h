...
public:
    ...
    void Syscall_Execute()
    {
        int virtAddr = machine->ReadRegister(4);
        char *ExecuteName = User2System(virtAddr, MaxFileLength + 1);
        ...
        if (strlen(ExecuteName) == 0 || (strlen(ExecuteName) >= MaxFileLength + 1))
        {
            printf("\nToo many characters or invalid character in this Execute name");
            DEBUG('a', "\nToo many characters or invalid character in this Execute name");
            machine->WriteRegister(2, -1);
            delete ExecuteName;
            return;
        ...
        OpenFile *execute = fileSystem->Open(ExecuteName);
        if (execute == NULL)
        {
            printf("\nError opening '%s'", ExecuteName);
            DEBUG('a', "\nError opening");
            machine->WriteRegister(2, -1);
            delete ExecuteName;
            return;
